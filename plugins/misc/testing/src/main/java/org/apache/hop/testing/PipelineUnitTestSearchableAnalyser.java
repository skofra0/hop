...
public class PipelineUnitTestSearchableAnalyser extends BaseMetadataSearchableAnalyser<PipelineUnitTest> implements ISearchableAnalyser<PipelineUnitTest> {
  ...
  public List<ISearchResult> search(ISearchable<PipelineUnitTest> searchable, ISearchQuery searchQuery) {
    ...
    matchProperty(searchable, results, searchQuery, "type", unitTest.getType() != null ? unitTest.getType().name() : null, null);
    matchProperty(searchable, results, searchQuery, "pipeline-filename", unitTest.getPipelineFilename(), null);
    ...
    matchProperty(searchable, results, searchQuery, "auto-open", unitTest.isAutoOpening() ? "Y" : "N", null);

    // Analyze the variables
    //
    for (VariableValue configurationVariable : unitTest.variableValues) {
      matchProperty(searchable, results, searchQuery, "Unit test variable name", configurationVariable.getKey(), null);
      matchProperty(searchable, results, searchQuery, "Unit test variable value", configurationVariable.getValue(), null);
    }

    // The database replacements...
    //
    for (PipelineUnitTestDatabaseReplacement replacement : unitTest.getDatabaseReplacements()) {
      matchProperty(searchable, results, searchQuery, "Unit test database replacement: original", replacement.getOriginalDatabaseName(), null);
      matchProperty(searchable, results, searchQuery, "Unit test database replacement: replacement", replacement.getReplacementDatabaseName(), null);
    }
