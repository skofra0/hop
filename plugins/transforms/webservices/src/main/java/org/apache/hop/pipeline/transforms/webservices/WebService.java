...
public class WebService extends BaseTransform<WebServiceMeta, WebServiceData> {
  ...
  public boolean processRow() throws HopException {
    ...
    if (first) {
      ...
      // Input from previous transforms, no longer getting any rows, call it a day...
      //
      if (vCurrentRow == null) {
        setOutputDone();
        return false;
  ...
  private void processRows(InputStream anXml, Object[] rowData, IRowMeta rowMeta, boolean ignoreNamespacePrefix, String encoding) throws HopException {
    ...
    try {
      ...
      // It's apparently a hassle to get the name in a consistent way, but we know it's the first
      // element node
      //
  ...
  private boolean getNodeValue(Object[] outputRowData, Node node, WebServiceField field, Transformer transformer, boolean singleRowScenario) throws HopException {
    ...
    if (node.getNodeType() == Node.TEXT_NODE || !field.isComplex()) {
      ...
      // Perhaps we can just ship the XML snippet over to the next transforms.
      //
      try {
