...
public final class WsdlOpParameter extends WsdlOpReturnType implements java.io.Serializable {
  ...
  /**
   * Create operation parameters whose types do not need to be unwrapped. Typically used for RPC
   * style parameters but also may be used for DOCUMENT style when parameter style is BARE.
   *
  ...
  /**
   * Create a new WsdlOpParameter for a simple schema type. For pararmeters of simple type, the name
   * of the parameter corresponds to the name of the message part which defines it.
   *
  ...
  /**
   * This method differs from the isArray(e) method in that it is checking a schema type to see if
   * it is an array type. In order to be an array type it must be a complex type which includes a
   * sequence of a single element which has it's minoccurs and/or maxoccures attributes set to
   * values which denote an array.
   *
