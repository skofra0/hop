...
public class SnowflakeBulkLoaderMeta extends BaseTransformMeta<SnowflakeBulkLoader, SnowflakeBulkLoaderData> {
  ...
  /**
   * JSON: Set if duplicate element names in the JSON be allowed. If true the last value for the name
   * is used.
   *
   ...
   *        started.
   * @param partNumber The partition number. Used when the pipeline is executed clustered, the number
   *        of the
   *        partition.
  ...
  /**
   * Check the transform to make sure it is valid. This is what is run when the user presses the check
   * pipeline
   * button in Hop
  ...
  public void check(
    ...
    if (prev != null && prev.size() > 0) {
      cr = new CheckResult(ICheckResult.TYPE_RESULT_OK, BaseMessages.getString(PKG, "SnowflakeBulkLoadMeta.CheckResult.FieldsReceived", "" + prev.size()), transformMeta);
      remarks.add(cr);
      ...
      if (errorFound) {
        errorMessage = BaseMessages.getString(PKG, "SnowflakeBulkLoadMeta.CheckResult.FieldsNotFound", errorMessage);
        cr = new CheckResult(ICheckResult.TYPE_RESULT_ERROR, errorMessage, transformMeta);
        remarks.add(cr);
      } else {
        cr = new CheckResult(ICheckResult.TYPE_RESULT_OK, BaseMessages.getString(PKG, "SnowflakeBulkLoadMeta.CheckResult.AllFieldsFound"), transformMeta);
        remarks.add(cr);
      }
    }
    ...
    if (input.length > 0) {
      cr = new CheckResult(ICheckResult.TYPE_RESULT_OK, BaseMessages.getString(PKG, "SnowflakeBulkLoadMeta.CheckResult.ExpectedInputOk"), transformMeta);
      remarks.add(cr);
    } else {
      cr = new CheckResult(ICheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(PKG, "SnowflakeBulkLoadMeta.CheckResult.ExpectedInputError"), transformMeta);
      remarks.add(cr);
    }
      ...
      try {
        ...
        cr =
            new CheckResult(
                ...
                BaseMessages.getString(PKG, "SnowflakeBulkLoadMeta.CheckResult.MappingValid", snowflakeBulkLoaderField.getStreamField()),
                transformMeta);
        remarks.add(cr);
        ...
        cr =
            new CheckResult(
                ...
                BaseMessages.getString(PKG, "SnowflakeBulkLoadMeta.CheckResult.MappingNotValid", snowflakeBulkLoaderField.getStreamField()),
                transformMeta);
        remarks.add(cr);
      }
    }
  ...
  public IRowMeta getRequiredFields(IVariables variables) throws HopException {
    ...
    if (connection != null) {
      ...
      try {
        ...
        throw new HopException(BaseMessages.getString(PKG, "SnowflakeBulkLoaderMeta.Exception.ErrorGettingFields"), e);
      } finally {
        db.disconnect();
      }
    } else {
  ...
  public String getCopyStatement(IVariables variables, List<String> filenames) throws HopFileException {
    ...
    } else if (onError.equals(ON_ERROR_CODES[ON_ERROR_SKIP_FILE]) || onError.equals(ON_ERROR_CODES[ON_ERROR_SKIP_FILE_PERCENT])) {
      ...
      if (prev != null && prev.size() > 0) {
        ...
        if (!Utils.isEmpty(targetTable)) {
          ...
          try {
            ...
            String schemaTable = databaseMeta.getQuotedSchemaTableCombination(variables, targetSchema, targetTable);
        ...
        retval.setError(BaseMessages.getString(PKG, "SnowflakeBulkLoaderMeta.GetSQL.NotReceivingAnyFields"));
      }
    } else {
