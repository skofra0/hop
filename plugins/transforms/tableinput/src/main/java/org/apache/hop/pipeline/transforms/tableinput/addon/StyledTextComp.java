...
public class StyledTextComp extends Composite {
  ...
  private void undo() {
    if (!undoStack.isEmpty()) {
      ...
      UndoRedoStack rro = new UndoRedoStack(urs.getCursorPosition(), urs.getReplacedText(), textWidget.getText(), urs.getEventLength(), urs.getType());
      bFullSelection = false;
      ...
      if (urs.getType() == UndoRedoStack.INSERT) {
        ...
        textWidget.setSelection(urs.getCursorPosition(), urs.getCursorPosition() + urs.getEventLength());
        if (textWidget.getSelectionCount() == textWidget.getCharCount()) {
          bFullSelection = true;
        }
      }
