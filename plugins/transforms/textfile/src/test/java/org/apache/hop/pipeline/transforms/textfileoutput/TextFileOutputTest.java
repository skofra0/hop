...
public class TextFileOutputTest {
  ...
  public void setUp() throws Exception {
    ...
    Mockito.when(
            transformMockHelper.logChannelFactory.create(
                Mockito.any(), Mockito.any(ILoggingObject.class)))
        .thenReturn(transformMockHelper.iLogChannel);
    Mockito.verify(transformMockHelper.iLogChannel, Mockito.never()).logError(Mockito.anyString());
    Mockito.verify(transformMockHelper.iLogChannel, Mockito.never())
        .logError(Mockito.anyString(), Mockito.any(Object[].class));
    Mockito.verify(transformMockHelper.iLogChannel, Mockito.never())
  ...
  private FileObject createTemplateFile() {
    String path =
        TestUtils.createRamFile(
            getClass().getSimpleName()
                + "/"
                + TEXT_FILE_OUTPUT_PREFIX
                + new Random().nextLong()
          ...
          for (Boolean endLineExists : BOOL_VALUE_LIST) {
            for (Boolean append : BOOL_VALUE_LIST) {
              try {
                resultFile =
                    helpTestInit(
                        fileExists, dataReceived, isDoNotOpenNewFileInit, endLineExists, append);
  ...
  private FileObject helpTestInit(
      Boolean fileExists,
      Boolean dataReceived,
      Boolean isDoNotOpenNewFileInit,
      Boolean endLineExists,
      Boolean append)
      throws Exception {
    FileObject f;
    String endLine = null;
  ...
  private List<Throwable> doOutput(
    ...
    Mockito.when(transformMockHelper.iTransformMeta.isDoNotOpenNewFileInit())
        .thenReturn(isDoNotOpenNewFileInit);
  ...
  public void testProcessRule_2() throws Exception {
    ...
    Mockito.when(transformMockHelper.iTransformData.getFileStreamsCollection())
        .thenCallRealMethod();
