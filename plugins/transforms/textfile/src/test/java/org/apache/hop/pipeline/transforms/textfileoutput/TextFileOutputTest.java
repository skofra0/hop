...
public class TextFileOutputTest {
  ...
  public class TextFileOutputTestHandler extends TextFileOutput<TextFileOutputMeta, TextFileOutputData> {
    ...
    TextFileOutputTestHandler(TransformMeta transformMeta, TextFileOutputMeta meta, TextFileOutputData data, int copyNr, PipelineMeta pipelineMeta, Pipeline pipeline) {
  ...
  public void setUp() throws Exception {
    transformMockHelper = new TransformMockHelper<>("TEXT FILE OUTPUT TEST", TextFileOutputMeta.class, TextFileOutputData.class);
    Mockito.when(transformMockHelper.logChannelFactory.create(Mockito.any(), Mockito.any(ILoggingObject.class))).thenReturn(transformMockHelper.iLogChannel);
  ...
  public void testCloseFileDataOutIsNullCase() {
    textFileOutput =
        new TextFileOutput(
            ...
            Mockito.mock(TextFileOutputData.class),
            0,
  ...
  private FileObject createTemplateFile() {
    String path = TestUtils.createRamFile(getClass().getSimpleName() + "/" + TEXT_FILE_OUTPUT_PREFIX + new Random().nextLong() + TEXT_FILE_OUTPUT_EXTENSION, new Variables());
  ...
  /**
   * Tests the RULE#1: If 'Do not create file at start' checkbox is cheked AND 'Add landing line of
   * file' is NOT set AND pipeline does not pass any rows to the file input transform, then NO
    ...
    Mockito
        .when(
            transformMockHelper.iTransformMeta.buildFilename(
                Mockito.anyString(), Mockito.anyString(), Mockito.any(IVariables.class), Mockito.anyInt(), Mockito.anyString(), Mockito.anyInt(), Mockito.anyBoolean(),
                Mockito.anyString(), Mockito.anyInt(), Mockito.anyBoolean(), Mockito.any(TextFileOutputMeta.class)))
  ...
  private List<Throwable> doOutput(
      TextFileField[] textFileField,
      List<Object[]> rows,
      String pathToFile,
      String endedLine,
    ...
    Mockito.when(rowSet.getRowWait(Mockito.anyInt(), Mockito.any(TimeUnit.class))).thenReturn(rows.isEmpty() ? null : rows.iterator().next());
  ...
  public void testEndedLineVar() throws Exception {
    ...
    TextFileOutput textFileOutput = new TextFileOutputTestHandler(transformMockHelper.transformMeta, meta, data, 0, transformMockHelper.pipelineMeta, transformMockHelper.pipeline);
  ...
  private void assertNotInvokedTwice(TextFileField field) {
    ...
    Mockito.verify(transform).containsSeparatorOrEnclosure(Mockito.any(byte[].class), Mockito.any(byte[].class), Mockito.any(byte[].class));
  }

  /**
   * File Exists=N Flag Set=N Add Header=Y Append=Y Result = File is created, header is written at
  ...
  public void testProcessRule_2() throws Exception {
    ...
    Mockito
        .when(
            transformMockHelper.iTransformMeta
                .buildFilename(TEXT_FILE_OUTPUT_PREFIX + TEXT_FILE_OUTPUT_EXTENSION, null, textFileOutputSpy, 0, null, 0, false, null, 0, true, transformMockHelper.iTransformMeta))
        .thenReturn(TEXT_FILE_OUTPUT_PREFIX + TEXT_FILE_OUTPUT_EXTENSION);
