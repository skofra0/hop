...
public class InsertUpdateMetaTest {
  ...
  public void setUp() {
    ...
    mockHelper = new TransformMockHelper<>("insertUpdate", InsertUpdateMeta.class, InsertUpdateData.class);
  ...
  public void setUpLoadSave() throws Exception {
    ...
    validatorFactory.registerValidator(
        validatorFactory.getName(List.class, InsertUpdateLookupField.class), new ListLoadSaveValidator<InsertUpdateLookupField>(new InsertUpdateLookupFieldLoadSaveValidator()));
    ...
    validatorFactory.registerValidator(
        validatorFactory.getName(List.class, InsertUpdateKeyField.class), new ListLoadSaveValidator<InsertUpdateKeyField>(new InsertUpdateKeyFieldLoadSaveValidator()));

    validatorFactory.registerValidator(
        validatorFactory.getName(InsertUpdateValue.class),
        new ObjectValidator<InsertUpdateValue>(validatorFactory, InsertUpdateValue.class, Arrays.asList("name", "rename"), new HashMap<String, String>() {
          {
    ...
    validatorFactory
        .registerValidator(validatorFactory.getName(List.class, InsertUpdateValue.class), new ListLoadSaveValidator<InsertUpdateValue>(new InsertUpdateValueLoadSaveValidator()));
  }
  ...
  public class InsertUpdateLookupFieldLoadSaveValidator implements IFieldLoadSaveValidator<InsertUpdateLookupField> {
    ...
    public InsertUpdateLookupField getTestObject() {
      ...
      InsertUpdateLookupField field =
          new InsertUpdateLookupField(UUID.randomUUID().toString(), UUID.randomUUID().toString(), new ArrayList<InsertUpdateKeyField>(), new ArrayList<InsertUpdateValue>());
    ...
    public boolean validateTestObject(InsertUpdateLookupField testObject, Object actual) {
      ...
      return new EqualsBuilder().append(testObject.getSchemaName(), another.getSchemaName()).append(testObject.getTableName(), another.getTableName())
          .append(testObject.getLookupKeys(), another.getLookupKeys()).append(testObject.getValueFields(), another.getValueFields()).isEquals();
    }
  }

  public class InsertUpdateValueLoadSaveValidator implements IFieldLoadSaveValidator<InsertUpdateValue> {
    final Random rand = new Random();

    @Override
    public InsertUpdateValue getTestObject() {

      InsertUpdateValue field = new InsertUpdateValue(UUID.randomUUID().toString(), UUID.randomUUID().toString());
  ...
  public class InsertUpdateKeyFieldLoadSaveValidator implements IFieldLoadSaveValidator<InsertUpdateKeyField> {
    final Random rand = new Random();

    @Override
    public InsertUpdateKeyField getTestObject() {

      InsertUpdateKeyField field = new InsertUpdateKeyField(UUID.randomUUID().toString(), UUID.randomUUID().toString(), "=", UUID.randomUUID().toString());
    ...
    public boolean validateTestObject(InsertUpdateKeyField testObject, Object actual) {
      ...
      return new EqualsBuilder().append(testObject.getKeyStream(), another.getKeyStream()).append(testObject.getKeyLookup(), another.getKeyLookup())
          .append(testObject.getKeyCondition(), another.getKeyCondition()).append(testObject.getKeyStream2(), another.getKeyStream2()).isEquals();
    }
  }
}
