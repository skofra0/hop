...
public class TransformMockUtil {
  ...
  public static <T extends ITransformMeta, V extends BaseTransform, D extends ITransformData>
      TransformMockHelper<T, D> getTransformMockHelper(Class<T> meta, Class<D> data, String name) {
    TransformMockHelper<T, D> transformMockHelper = new TransformMockHelper<>(name, meta, data);
    when(transformMockHelper.logChannelFactory.create(
            ArgumentMatchers.any(), ArgumentMatchers.any(ILoggingObject.class)))
        .thenReturn(transformMockHelper.iLogChannel);
    when(transformMockHelper.logChannelFactory.create(ArgumentMatchers.any()))
        .thenReturn(transformMockHelper.iLogChannel);
  ...
  public static <T extends BaseTransform, K extends ITransformMeta, D extends ITransformData>
      T getTransform(
          Class<T> transformClass,
          Class<K> transformMetaClass,
          Class<D> transformDataClass,
    ...
    return TransformMockUtil.getTransform(
        ...
        TransformMockUtil.getTransformMockHelper(
            transformMetaClass, transformDataClass, transformName));
  }
}
