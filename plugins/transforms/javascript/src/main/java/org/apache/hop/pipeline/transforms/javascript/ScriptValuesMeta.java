...
public class ScriptValuesMeta extends BaseTransformMeta<ScriptValues, ScriptValuesData> {
  ...
  public ScriptValuesMeta() {
    ...
    try {
      if (Objects.isTrue(
          System.getProperty("SUPPORT_JAVASCRIPT_ADDONS", "N"))) { // DEEM-MOD performance
  ...
  public boolean getValue(Scriptable scope, int i, Value res, StringBuffer message) {
    ...
    if (fieldname[i] != null && fieldname[i].length() > 0) {
      ...
      message.append(
          BaseMessages.getString(
              PKG,
              "ScriptValuesMetaMod.CheckResult.RetrievedValue",
              fieldname[i],
              res.toStringMeta()));
    } else {
      message.append(
          BaseMessages.getString(
              PKG, "ScriptValuesMetaMod.CheckResult.ValueIsEmpty", String.valueOf(i)));
      errorFound = true;
    }
