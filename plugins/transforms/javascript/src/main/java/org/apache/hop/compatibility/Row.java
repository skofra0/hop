 ...
 * Note that most methods in this class assume that a value with a certain name only occurs once in
 * the Row.
 *
 * @see Value
 */
public class Row implements IXml, Comparable<Row>, Serializable {
  ...
  private List<ValueUsedListener> usedValueListeners = new ArrayList<>();
  ...
  /**
   * Creates a new row as a duplicate of the given row. The values are copied one by one into new
   * values.
   *
  ...
  public static Row createOriginalRow(IRowMeta rowMeta, Object[] rowData) throws HopValueException {
    Row row = new Row();
  ...
  /**
   * Merge the values of row r to this Row. Merge means: only the values that are not yet in the row
   * are added (comparing on the value name).
   *
  ...
  /**
   * Merge the data of row r to this Row. That means: All fields in row r that do exist in this row
   * (same name and same type) and have non-empty values will have their values written into this
   * row, if the value of that field is empty in this row.
   *
   * @param r The row to be merged with this row
   */
  ...
  public void print() {
    ...
    for (i = 0; i < size(); i++) {
      System.out.println(
          "Element[" + i + "] = [" + getValue(i).getName() + "] = " + getValue(i).toString());
    }
  }
  ...
  /** Marks this row as to be ignored by the next transforms. */
  public void setIgnore() {
    ignore = true;
  }

  /**
   * Marks this row as to be ignored or not by the next transforms.
   *
   ...
   * @throws HopFileException if an error occurs.
   */
  public void write(DataOutputStream dos) throws HopFileException {
    try {
      writeObj(dos);
    } catch (Exception e) {
      throw new HopFileException(BaseMessages.getString(PKG, "Row.ErrorWritingRow"), e);
    }
  }
   ...
   */
  public Row(DataInputStream dis) throws HopFileException {
    try {
      readObj(dis);
    } catch (EOFException e) {
      throw new HopEofException(BaseMessages.getString(PKG, "Row.EndOfFileReached"), e);
    } catch (Exception e) {
      throw new HopFileException(BaseMessages.getString(PKG, "Row.ErrorReadingRowData"), e);
    }
  }
   ...
   *
   * @param dis The DataInputStream to read from
   * @param meta The description (name, type, length, precision) of the values to be read (the same
   *     number of values are read)
   * @throws HopFileException if the row couldn't be created by reading from the data input stream.
   */
  public Row(DataInputStream dis, Row meta) throws HopFileException {
   ...
   * @throws HopFileException if the row couldn't be created by reading from the data input stream.
   */
  public Row(DataInputStream dis, int size, Row meta) throws HopFileException {
    try {
      // get all values in the row
      for (int i = 0; i < size; i++) {
      ...
      throw new HopEofException(BaseMessages.getString(PKG, "Row.EndOfFileReadingRow"), e);
    } catch (Exception e) {
      throw new HopFileException(BaseMessages.getString(PKG, "Row.RowError"), e);
    }
  }
  ...
  /**
   * Compare 2 rows with each other using certain values in the rows and also considering an
   * ascending clause.
   *
   * @param r The row to compare with
   * @param fieldnrs The indexes of the values to compare
   * @param ascending an entry for each value to compare where true means and normal compare, false
   *     the reverse.
   ...
   */
  public int compare(
      Row r, int[] fieldnrs1, int[] fieldnrs2, boolean[] ascending, boolean[] caseInsensitive) {
    int retval = 0;
    int i;
  ...
  /**
   * Compare 2 rows with each other using one value in the rows and also considering an ascending
   * clause.
   *
   * @param r The row to compare with
  ...
  /**
   * Compare 2 complete rows of values with each other. Strings are compared in a case insensitive
   * way
   *
   * @param r the row to compare with
  ...
  /**
   * Returns an exact copy of this row. On purpose not the real clone() method (notice the uppercase
   * 'C') because of (rhino) javascript problems with it.
   *
  ...
  /**
   * Checks whether or not the row is empty A row is empty if all the values in the row are null A
   * row is empty if there are no values in the row.
   *
  ...
  public String getXml(IVariables variables) {
    ...
    for (int i = 0; i < size(); i++) {
      xml.append(getValue(i).getXml(variables));
    }
  ...
  public static final void sortRows(List<Row> rows, int[] fieldNrs, boolean[] ascDesc) {
    ...
    Comparator<Row> comparator = (one, two) -> one.compare(two, fieldNumbers, ascending);
