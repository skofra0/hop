...
public class XmlInputStream extends BaseTransform<XmlInputStreamMeta, XmlInputStreamData> {
  ...
  public boolean processRow() throws HopException {
    ...
    // limit has been reached: stop now. (not exact science since some attributes could be mixed
    // within the last row)
  ...
  /**
   * If there are incoming hops: Retrieves filenames from all rows that pass through this transform
   * using field name selected from incoming hops. If no input field is chosen and there is some
   * manually typed text it will try to open a file using this text (for compatibility reasons) If
   * there is no incoming hops it will simply try to open file with path provided in transform
   * dialog
   *
   * @throws HopException
   */
  ...
  private void putRowOut(Object[] r) throws HopTransformException, HopValueException {
    ...
    if (data.pos_xmlFilename != -1) {
      r[data.pos_xmlFilename] = String.valueOf(data.filenames[(data.filenr - 1)]);
    }
    if (data.pos_xmlRow_number != -1) {
      r[data.pos_xmlRow_number] = data.rowNumber;
    }
      ...
      case XMLStreamConstants.START_ELEMENT:
        ...
        if (meta.isEnableNamespaces()) {
          ...
          if (Utils.isEmpty(prefix)) {
            xmlDataName = e.asStartElement().getName().getLocalPart();
          } else { // add namespace prefix:
            xmlDataName = prefix + ":" + e.asStartElement().getName().getLocalPart();
          }
        } else {
          xmlDataName = e.asStartElement().getName().getLocalPart();
        }
        if (data.pos_xml_dataName >= 0) {
          outputRowData[data.pos_xml_dataName] = xmlDataName;
        }
        // store the name
      ...
      case XMLStreamConstants.CHARACTERS, XMLStreamConstants.CDATA:
        ...
        if (data.pos_xml_dataValue >= 0) {
          if (meta.isEnableTrim()) {
            ...
            xmlDataValue = Const.trim(xmlDataValue);
          }
  ...
  /**
   * Returns the qualified name in the format: <code>prefix:localPart</code> if the prefix is
   * present otherwise just <code>localPart</code>
   *
