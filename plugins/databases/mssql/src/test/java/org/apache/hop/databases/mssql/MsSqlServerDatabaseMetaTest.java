...
public class MsSqlServerDatabaseMetaTest {
  ...
  public void testSqlStatements() {
    ...
    assertEquals(
        "SELECT top 0 * FROM FOO WITH (UPDLOCK, HOLDLOCK);" + lineSep + "SELECT top 0 * FROM BAR WITH (UPDLOCK, HOLDLOCK);" + lineSep,
    ...
    assertEquals("select o.name from sysobjects o, sysusers u where  xtype in ( 'FN', 'P' ) and o.uid = u.uid order by o.name", nativeMeta.getSqlListOfProcedures());
  ...
  public void testGetFieldDefinition() throws Exception {
    ...
    assertEquals("FLOAT(53)", nativeMeta.getFieldDefinition(new ValueMetaNumber("BAR", 10, -7), "", "", false, false, false)); // Bug here - invalid SQL
    ...
    assertEquals("BIGINT PRIMARY KEY IDENTITY(0,1)", nativeMeta.getFieldDefinition(new ValueMetaInteger("BAR"), "", "BAR", true, false, false));
    assertEquals("BIGINT PRIMARY KEY", nativeMeta.getFieldDefinition(new ValueMetaNumber("BAR"), "", "BAR", false, false, false));
    assertEquals("VARBINARY(MAX)", nativeMeta.getFieldDefinition(new ValueMetaBinary(), "", "BAR", false, false, false));
    assertEquals("VARBINARY(MAX)", nativeMeta.getFieldDefinition(new ValueMetaBinary("BAR"), "", "BAR", false, false, false));
  }
  ...
  public void testCheckIndexExists() throws Exception {
    String expectedSQL =
        "select i.name table_name, c.name column_name from     sysindexes i, sysindexkeys k, syscolumns c where    i.name = 'FOO' AND      i.id = k.id AND      i.id = c.id AND      k.colid = c.colid "; // yes,
                                                                                                                                                                                                          // variables
                                                                                                                                                                                                          // at
                                                                                                                                                                                                          // the
                                                                                                                                                                                                          // end
   ...
   * assertFalse(
   * databaseMeta.databaseForBothDbInterfacesIsTheSame( mssqlServerDatabaseMeta,
   * mssqlServerDatabaseMeta ) );
   * }
   */
   ...
   * 
   * assertFalse( databaseMeta.databaseForBothDbInterfacesIsTheSame( mssqlServerDatabaseMeta,
   * oracleDatabaseMeta ) );
   * }
   */
   ...
   * @Test
   * public void
   * databases_WithDifferentDbConnTypes_AreTheSame_IfOneConnTypeIsSubsetOfAnother_3LevelHierarchy() {
   ...
   * assertTrue(
   * databaseMeta
   * .databaseForBothDbInterfacesIsTheSame( mssqlServerDatabaseMeta,
   * mssqlServerNativeDatabaseMetaChild ) );
   * }
   */
