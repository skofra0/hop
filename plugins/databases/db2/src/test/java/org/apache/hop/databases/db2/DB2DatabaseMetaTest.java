...
public class DB2DatabaseMetaTest {
  ...
  public void testSqlStatements() {
    ...
    assertEquals(
        "ALTER TABLE FOO ADD COLUMN BAR CLOB", nativeMeta.getAddColumnStatement("FOO", new ValueMetaString("BAR", nativeMeta.getMaxVARCHARLength() + 2, 0), "", false, "", false));
    ...
    assertEquals(
        "ALTER TABLE FOO DROP COLUMN BAR" + lineSep + ";" + lineSep + "ALTER TABLE FOO ADD COLUMN BAR VARCHAR(15)",
  ...
  public void testGetFieldDefinition() {
    ...
    assertEquals("DECIMAL(12)", nativeMeta.getFieldDefinition(new ValueMetaBigNumber("FOO", 12, 0), "", "", false, false, false)); // Pretty sure this is a bug - should be an
                                                                                                                                   // Integer here.
    ...
    assertEquals(
        String.format("VARCHAR(%d)", realMinBeforeCLOB - 1), nativeMeta.getFieldDefinition(new ValueMetaString("FOO", realMinBeforeCLOB - 1, 0), "", "", false, false, false));
    assertEquals("VARCHAR()", nativeMeta.getFieldDefinition(new ValueMetaString("FOO", 0, 0), "", "", false, false, false)); // Definitely a bug here - VARCHAR() is not valid SQL
                                                                                                                             // anywhere . . .

    // Binary Stuff . . .
    assertEquals(
        String.format("BLOB(%d)", realMaxBeforeCLOB + 1), nativeMeta.getFieldDefinition(new ValueMetaBinary("FOO", realMaxBeforeCLOB + 1, 0), "", "", false, false, false));
