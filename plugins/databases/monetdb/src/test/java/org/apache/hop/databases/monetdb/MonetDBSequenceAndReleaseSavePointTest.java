...
public class MonetDBSequenceAndReleaseSavePointTest {
  ...
  IDatabase db = new MonetDBDatabaseMeta();
  ...
  public void testSequenceSupport() {
    ...
    assertEquals("SELECT next_value_for( 'sys', 'sequence_name' )", db.getSqlNextSequenceValue(sequenceName));
    assertEquals("SELECT get_value_for( 'sys', 'sequence_name' )", db.getSqlCurrentSequenceValue(sequenceName));
    assertEquals("SELECT name FROM sys.sequences", db.getSqlListOfSequences());
    assertEquals("SELECT * FROM sys.sequences WHERE name = 'sequence_name'", db.getSqlSequenceExists(sequenceName));
  }
