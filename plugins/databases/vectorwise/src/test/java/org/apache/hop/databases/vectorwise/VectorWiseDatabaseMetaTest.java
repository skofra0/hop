...
public class VectorWiseDatabaseMetaTest {
  ...
  public void testIngresOverrides() {
    ...
    assertEquals(
        "jdbc:ingres://FOO:VW7/WIBBLE", nativeMeta.getURL("FOO", "", "WIBBLE")); // Empty bit
    assertEquals(
        "jdbc:ingres://FOO:VW7/WIBBLE", nativeMeta.getURL("FOO", "-1", "WIBBLE")); // Empty bit
    ...
    assertEquals(
        "ALTER TABLE FOO DROP COLUMN BAR" + System.getProperty("line.separator"),
  ...
  public void testGetFieldDefinition() {
    ...
    assertEquals(
        ...
        nativeMeta.getFieldDefinition(
            new ValueMetaNumber("FOO", 10, 0), "FOO", "", true, false, false));
    assertEquals(
        "GENERATED ALWAYS AS IDENTITY START WITH 1 INCREMENT BY 1",
        nativeMeta.getFieldDefinition(
            new ValueMetaInteger("FOO", 10, 0), "", "FOO", true, false, false));
    assertEquals(
    ...
    assertEquals(
        "VARCHAR(32000)",
        nativeMeta.getFieldDefinition(
            new ValueMetaString("FOO", 32000, 0), "", "", false, false, false));
    assertEquals(
        "VARCHAR(32000)",
        nativeMeta.getFieldDefinition(
            new ValueMetaString("FOO", 32050, 0), "", "", false, false, false));
    assertEquals(
        "VARCHAR(9999)",
    ...
    assertEquals(
        "VARCHAR(9999)",
        nativeMeta.getFieldDefinition(
            new ValueMetaString("FOO", -34, 0), "", "", false, false, false));
