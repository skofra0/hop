 ...
 *
 * <p>This class can be declared and passed to the current File-system manager
 *
 * <p>E.g....
 *
 * <pre><code>
...
public class AzureFileProvider extends AbstractOriginatingFileProvider {
  ...
  /**
   * In the case that we are not sent FileSystemOptions object, we need to have one handy.
   *
   * @return
   */
  ...
  protected FileSystem doCreateFileSystem(FileName rootName, FileSystemOptions fileSystemOptions)
    ...
    AzureFileName azRootName = (AzureFileName) rootName;

    AzureFileSystem fileSystem;

    FileSystemOptions resolvedFileSystemOptions =
        (fileSystemOptions != null) ? fileSystemOptions : getDefaultFileSystemOptions();
    UserAuthenticator ua =
        DefaultFileSystemConfigBuilder.getInstance()
            .getUserAuthenticator(resolvedFileSystemOptions);
    ...
    try {
      ...
      fileSystem =
          new AzureFileSystem(
              azRootName, blobContainerAsyncClient, blobContainerClient, fileSystemOptions);
    } finally {
