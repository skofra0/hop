...
/**
 * The main FileObject class in this provider. It holds most of the API callbacks for the provider.
 */
public class AzureFileObject extends AbstractFileObject<AzureFileSystem> {

  private static final Logger log = LoggerFactory.getLogger(AzureFileObject.class);
  ...
  private static final long STREAM_BUFFER_SIZE_MB =
      DEFAULT_UPLOAD_BLOCK_SIZE_MB * MEGABYTES_TO_BYTES_MULTIPLIER;
   ...
   */
  public AzureFileObject(final AbstractFileName fileName, final AzureFileSystem fileSystem) {
   ...
   *
   * <p>The File Object should be reusable after <code>attach()</code> call.
   */
  @Override
  ...
  /**
   * Callback for checking the type of the current FileObject. Typically can be of type... FILE for
   * regular remote files FOLDER for regular remote containers IMAGINARY for a path that does not
   * exist remotely.
   *
  ...
  protected FileType doGetType() {
    ...
    AzureFileName fileName = (AzureFileName) getName();

    // file type IMAGINARY check is required because in case of place holder type object file type
    // would be IMAGINARY so
    // that needs to be corrected once it gets imported.
    // second reason behind this check is, this.isAttached and super.attached properties are not in
    // sync when this
    // .doAttached() called directly so while closing object (fleObject.close()) internally it calls
    // detach() method to
    // detach the object but it finds supper.attached to false and return from there without
   ...
   * FileType#FOLDER}. The return value of this method is cached, so the implementation can be
   * expensive.<br>
   *
  ...
  /**
   * Callback for handling create folder requests. Since there are no folders in Azure Cloud Storage
  ...
  public FileObject[] getChildren() throws FileSystemException {
    ...
    try {
      // Folders which are copied from other folders, have type = IMAGINARY. We can not throw
      // exception based on folder
      ...
      for (FileObject srcFile : files) {
        ...
        try {
          if (srcFile.getType().hasChildren()) {
            ...
            AzureFileObject destAzFile = (AzureFileObject) destFile;

            String url =
                ((AzureFileObject) srcFile).getSignedUrl(TWENTY_FOUR_HOURS_IN_SEC).toString();

            if (srcFile.getContent().getSize() > BLOB_COPY_THRESHOLD_MB) {
              SyncPoller<BlobCopyInfo, Void> poll =
                  destAzFile.blobClient.beginCopy(url, Duration.ofSeconds(1));
              ...
              if (pollResponse.getStatus() != LongRunningOperationStatus.SUCCESSFULLY_COMPLETED) {
                ...
                throw new FileSystemException(
                    "vfs.provider/copy-file.error", exception, srcFile, destFile);
              }
            } else {
  ...
  private void doCopyFromStream(FileObject srcFile, FileObject destFile) throws Exception {
    ...
    try {
      ...
      try (BlobOutputStream bos =
              destBlobClient
                  .getBlockBlobClient()
                  .getBlobOutputStream(opts, null, null, null, requestConditions);
      ...
      ((AzureFileObject) destFile).doGetType();
    } finally {
      destFile.close();
      srcFile.close();
    }
  ...
  /**
   * Returns the block size depending on the size of the file to be uploaded. A default block size
   * of 4MB is used until the file size is larger than 4mb * 50000, after this block size is scaled
   * so that 50000 blocks are used.
   *
  ...
  protected long getBlockSize(long fileSize) throws FileSystemException {
    ...
    long dynamicBlockSizeThreshold =
        (DEFAULT_UPLOAD_BLOCK_SIZE_MB * AZURE_MAX_BLOCKS) * MEGABYTES_TO_BYTES_MULTIPLIER;
  ...
  private boolean canCopyServerSide(FileObject sourceFileObject, FileObject destinationFileObject) {
    ...
    if (!(sourceFileObject instanceof AzureFileObject)
        || !(destinationFileObject instanceof AzureFileObject)) {
      return false;
    }

    AzureFileObject azSourceFileObject = (AzureFileObject) sourceFileObject;
    AzureFileObject azDestinationFileObject = (AzureFileObject) destinationFileObject;
  ...
  /**
   * Returns false to reply on copyFrom method in case moving/copying file within same azure
   * container
   *
  ...
  public URL getSignedUrl(int durationSec) throws Exception {
    ...
    BlobServiceSasSignatureValues signatureValues =
        new BlobServiceSasSignatureValues(offsetDateTime, sasPermission);
   ...
   */
  private String getAccountName(AzureFileObject azFileObject) {

    AzureFileSystem azFileSystem = (AzureFileSystem) azFileObject.getFileSystem();
