...
public class AzureFileProvider extends AbstractOriginatingFileProvider {
  ...
  protected FileSystem doCreateFileSystem(FileName fileName, FileSystemOptions fileSystemOptions)
    ...
    var fsOptions = fileSystemOptions != null ? fileSystemOptions : getDefaultFileSystemOptions();
    ...
    try {
      ...
      var config = AzureConfigSingleton.getConfig();

      if (StringUtils.isEmpty(config.getAccount())) {
        throw new FileSystemException("Please configure the Azure account");
      }
      if (StringUtils.isEmpty(config.getKey())) {
        throw new FileSystemException("Please configure the Azure key");
      }
