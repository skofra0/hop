...
public class PipelineLoggingExtensionPoint implements IExtensionPoint<IPipelineEngine<PipelineMeta>> {
  ...
  private void logStartOfPipeline(final ILogChannel log, final Session session, final NeoConnection connection, final IPipelineEngine<PipelineMeta> pipeline) throws HopException {
    log.logDetailed("Logging execution start of pipeline to Neo4j connection : " + connection.getName());
    ...
    synchronized (session) {
      session.writeTransaction((TransactionWork<Void>) transaction -> {
        try {
          ...
          pipelinePars.put("executionStart", new SimpleDateFormat("yyyy/MM/dd'T'HH:mm:ss").format(startDate));
    ...
    synchronized (session) {
      session.writeTransaction((TransactionWork<Void>) transaction -> {
        try {
          ...
          pipelinePars.put("type", EXECUTION_TYPE_PIPELINE);
          pipelinePars.put("id", channel.getLogChannelId());
          pipelinePars.put("executionEnd", new SimpleDateFormat("yyyy/MM/dd'T'HH:mm:ss").format(endDate));
  ...
  private void logHierarchy(final ILogChannel log, final Session session, final NeoConnection connection, final List<LoggingHierarchy> hierarchies, String rootLogChannelId) {
    synchronized (session) {
      session.writeTransaction((TransactionWork<Void>) transaction -> {
