    ...
    try {
      ...
      while (pos < length) {
        ...
        if (lenEncl > 0 && line.substring(from, from + lenEncl).equalsIgnoreCase(enclosure)) {
          if (log.isRowLevel()) {
            log.logRowlevel(BaseMessages.getString(PKG, "TextFileLineUtil.Log.ConvertLineToRowTitle"), BaseMessages.getString(PKG, "TextFileLineUtil.Log.ConvertLineToRow", line.substring(from, from + lenEncl)));
          }
          enclFound = true;
          int p = from + lenEncl;
        ...
        if (enclFound) {
          ...
          if (log.isRowLevel()) {
            log.logRowlevel(BaseMessages.getString(PKG, "TextFileLineUtil.Log.ConvertLineToRowTitle"), BaseMessages.getString(PKG, "TextFileLineUtil.Log.EnclosureFieldFound", "" + pol));
          }
        } else {
          pol = line.substring(from, next);
          if (log.isRowLevel()) {
            log.logRowlevel(BaseMessages.getString(PKG, "TextFileLineUtil.Log.ConvertLineToRowTitle"), BaseMessages.getString(PKG, "TextFileLineUtil.Log.NormalFieldFound", "" + pol));
          }
        }
