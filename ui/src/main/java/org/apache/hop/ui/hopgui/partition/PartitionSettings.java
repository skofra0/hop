...
public class PartitionSettings {
  ...
  public PartitionSettings(
      ...
      MetadataManager<PartitionSchema> schemaManager) {
    ...
    System.arraycopy(
        TransformPartitioningMeta.methodDescriptions,
        0,
        options,
        0,
        TransformPartitioningMeta.methodDescriptions.length);
    System.arraycopy(
        TransformPartitioningMeta.methodCodes,
        0,
        codes,
        0,
        TransformPartitioningMeta.methodCodes.length);
  }

  public void fillOptionsAndCodesByPlugins(List<IPlugin> plugins) {
    int pluginIndex = 0;
    for (IPlugin plugin : plugins) {
      options[TransformPartitioningMeta.methodDescriptions.length + pluginIndex] =
  ...
  public String getMethodByMethodDescription(String methodDescription) {
    String method =
        TransformPartitioningMeta.methodCodes[TransformPartitioningMeta.PARTITIONING_METHOD_NONE];
  ...
  public List<PartitionSchema> getSchemas() {
    try {
      ...
      e.printStackTrace(
          System.out); // TODO: properly throw exception, don't eat exception like this!!!
      return Collections.emptyList();
    }
  }
