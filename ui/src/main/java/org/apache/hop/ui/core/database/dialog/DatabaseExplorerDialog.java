...
public class DatabaseExplorerDialog extends Dialog {
  ...
  private String getSchemaTable() {
    ...
    if (ti.length == 1) {
      ...
      if (parent != null) {
        ...
        {
          String tab = null;
          if (schemaName.equalsIgnoreCase(STRING_TABLES) || schemaName.equalsIgnoreCase(STRING_VIEWS) || schemaName.equalsIgnoreCase(STRING_SYNONYMS) || (schemaName != null && schemaName.length() == 0)) {
            tab = tableName;
          } else {
  ...
  public void previewTable(String tableName, boolean asklimit) {
    ...
    GetPreviewTableProgressDialog pd = new GetPreviewTableProgressDialog(shell, variables, dbMeta, null, tableName, limit);
  ...
  public void getDDLForOther(String tableName) {
    if (databases != null) {
      ...
      try {
        ...
        EnterSelectionDialog enterSelectionDialog = new EnterSelectionDialog(shell, connectionNames, BaseMessages.getString(PKG, "DatabaseExplorerDialog.TargetDatabase.Title"), BaseMessages.getString(PKG, "DatabaseExplorerDialog.TargetDatabase.Message"));
