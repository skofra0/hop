...
public class StyledTextComp2Replace extends org.eclipse.swt.widgets.Dialog {
  ...
  private static Class<?> PKG = StyledTextComp2Replace.class;
  ...
  public void open() {
          ...
          public void handleEvent(Event e) {
            if (text.getSelectionCount() < 1) {
              if (!findText()) {
                MessageBox messageBox = new MessageBox(sShell, SWT.ICON_INFORMATION | SWT.OK);
                messageBox.setText(
                    BaseMessages.getString(PKG, "Widget.Styled.CompReplace.FindItem"));
                messageBox.setMessage(
                    BaseMessages.getString(
                        PKG, "Widget.Styled.CompReplace.ItemNotFound", searchText.getText()));
                messageBox.open();
    ...
    replaceText.addModifyListener(
        new ModifyListener() {
          public void modifyText(ModifyEvent e) {
            if (replaceText.getText() != null
                && replaceText.getText().length() > 0
                && btnNext.isEnabled()) {
