...
public class MetadataContextHandler implements IGuiContextHandler {
  ...
  public List<GuiAction> getSupportedActions() {
    ...
    GuiAction deleteAction =
        new GuiAction("DELETE_" + hopMetadata.name(), GuiActionType.Delete, hopMetadata.name(), "After confirmation this deletes a " + hopMetadata.name() + " : " + hopMetadata.description(), hopMetadata.image(), (shiftClicked, controlClicked, parameters) -> metadataManager.deleteMetadata());
    ...
    if (metadataObjectClass.isAssignableFrom(DatabaseMeta.class)) {
      GuiAction databaseClearCacheAction = new GuiAction("DATABASE_CLEAR_CACHE", GuiActionType.Custom, BaseMessages.getString(PKG, "HopGui.Context.Database.Menu.ClearDatabaseCache.Label"), BaseMessages.getString(PKG, "HopGui.Context.Database.Menu.ClearDatabaseCache.Tooltip"), null,
          (shiftClicked, controlClicked, parameters) -> DbCache.getInstance().clear((String) parameters[0]));
      newAction.setClassLoader(metadataObjectClass.getClassLoader());
      newAction.setCategory("Metadata");
