...
public interface IRowMeta extends Cloneable {
  ...
  /**
   * Add a metadata value, extends the array if needed. If a value with the same name already
   * exists, it gets renamed.
   *
  ...
  /**
   * Determines whether a value in a row is null. A value is null when the object is null. As such,
   * you can just as good write dataRow[index]==null in your code.
   *
   ...
   * unchanged. The values that are in the row are renamed to name[2], name[3], etc.
   *
  ...
  /**
   * Merge the values of row r to this Row. The values that are not yet in the row are added
   * unchanged. The values that are in the row are renamed to name_1, name_2, etc.
   *
   * @param r The row to be merged with this row
   */
  ...
  /**
   * Write a serialized version of the supplied data to the outputStream (based on the metadata but
   ...
   * ['2008-01-01:12:30'] and ['2008-01-01:00:00'] that use a format object to change the value (as
   * Date yyyy-MM-dd), the hashCodes will be different resulting in the two rows not being
   * considered equal via the hashCode even though compare() or equals() might consider them to be.
   *
