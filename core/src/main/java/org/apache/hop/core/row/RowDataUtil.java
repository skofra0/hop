...
public class RowDataUtil {
  ...
  /**
   * Allocate a new Object array. However, over allocate by a constant factor to make adding values
   * faster.
   *
  ...
  /**
   * Resize an object array making it bigger, over allocate, return the original array if there's
   * enough room.
   *
   * @param objects
   * @param newSize
  ...
  /**
   * Resize an object array making it bigger, over allocate, always create a copy of the original
   * array, even if there's enough room in the old one.
   *
  ...
  /**
   * Remove an item from an Object array. This is a slow operation, later we want to just flag this
   * object and discard it at the next resize. The question is of-course if it makes that much of a
