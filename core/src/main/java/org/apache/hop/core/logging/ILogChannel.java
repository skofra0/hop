...
public interface ILogChannel {
  ...
  /**
   * Add a snapshot to the metrics system for this log channel at the time of invocation. This will
   * process the value depending on the type of metric specified. For example, for IMetrics.look up
   * the maximum value in the metrics and replace it if the new value is higher. The snapshot date
   * will be retained in that case.
   *
   * @param metric The metric to use (ex. connect to a database)
   * @param value the value to store
   */
  void snap(IMetrics metric, long... value);

  /**
   * Add a maximum snapshot to the metrics system for this log channel at the time of invocation.
   * This will look up the maximum value in the metrics and replace it if the new value is higher.
   * The snapshot date will be retained in that case.
   *
   * @param metric The metric to use (ex. connect to a database)
   * @param subject The subject (ex. the name of the database)
   * @param value the value to store
   */
  void snap(IMetrics metric, String subject, long... value);
}
