...
public class DatabaseConnectingTest {
  ...
  public void connect_GroupIsEqual_Consequently() throws Exception {
    ...
    db1.setConnectionGroup(GROUP);
    db1.connect();

    DatabaseStub db2 = createStubDatabase(shared);
    db2.setConnectionGroup(GROUP);
    db2.connect();
  ...
  public void connect_TwoGroups() throws Exception {
    try {
      ...
      db11.setConnectionGroup(GROUP);
      db11.connect();
      db12.setConnectionGroup(GROUP);
      db12.connect();

      db21.setConnectionGroup(ANOTHER_GROUP);
      db21.connect();
      db22.setConnectionGroup(ANOTHER_GROUP);
      db22.connect();
