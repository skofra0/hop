...
public class SqlScriptParserTest {
  ...
  public void testSplit() {
    ...
    assertEquals(
        Collections.singletonList("CREATE TABLE test1 (col1 STRING) TBLPROPERTIES (\"prop1\" = \"my\\\"value\")"),
    ...
    assertEquals(Collections.singletonList("SELECT 'test\\';SELECT 1"), sqlScriptParser.split("SELECT 'test\\';SELECT 1"));
    assertEquals(
        Arrays.asList("create table pdi13654 (col1 string) TBLPROPERTIES (\"quoteChar\"=\"\\\"\", \"escapeChar\"=\"\\\\\")", "SELECT 1"),
  ...
  public void testRemoveComments() {
    ...
    assertEquals(
        ...
        sqlScriptParser.removeComments("SELECT /* my data*/ col1, col2, col3 FROM account WHERE name = 'Apache Hop'"));
    assertEquals(
        "SELECT /*+ ORACLE hint*/ col1, col2, col3 FROM account WHERE name = 'Apache Hop'",
        sqlScriptParser.removeComments("SELECT /*+ ORACLE hint*/ col1, col2, col3 FROM account WHERE name = 'Apache Hop'"));
    assertEquals(
