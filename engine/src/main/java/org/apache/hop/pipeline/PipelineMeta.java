...
public class PipelineMeta extends AbstractMeta implements IXml, Comparator<PipelineMeta>, Comparable<PipelineMeta>, Cloneable, IResourceExport, IHasFilename {
  ...
  public static final String[] descTypeUndo =
      {"", BaseMessages.getString(PKG, "PipelineMeta.UndoTypeDesc.UndoChange"), BaseMessages.getString(PKG, "PipelineMeta.UndoTypeDesc.UndoNew"), BaseMessages.getString(PKG, "PipelineMeta.UndoTypeDesc.UndoDelete"), BaseMessages.getString(PKG, "PipelineMeta.UndoTypeDesc.UndoPosition")};
  ...
  public IRowMeta getPrevTransformFields(IVariables variables, TransformMeta transformMeta, final String transformName, IProgressMonitor monitor) throws HopTransformException {
    ...
    if (LogChannel.GENERAL.isDebug()) {
      LogChannel.GENERAL.logDebug(BaseMessages.getString(PKG, "PipelineMeta.Log.FromTransformALookingAtPreviousTransform", transformMeta.getName(), String.valueOf(nrPrevTransforms)));
    }
  ...
  public String getXml(IVariables variables) throws HopException {
    ...
    //
    xml.append("    ").append(XmlHandler.addTagValue("capture_transform_performance", capturingTransformPerformanceSnapShots));
    xml.append("    ").append(XmlHandler.addTagValue("transform_performance_capturing_delay", transformPerformanceCapturingDelay));
    xml.append("    ").append(XmlHandler.addTagValue("transform_performance_capturing_size_limit", transformPerformanceCapturingSizeLimit));
  ...
  public PipelineMeta(String fname, IHopMetadataProvider metadataProvider, boolean setInternalVariables, IVariables parentVariableSpace) throws HopXmlException, HopMissingPluginsException {
   ...
   * @throws HopMissingPluginsException in case missing plugins were found (details are in the
   *         exception in that case) */
  public PipelineMeta(InputStream xmlStream, IHopMetadataProvider metadataProvider, boolean setInternalVariables, IVariables parentVariableSpace) throws HopXmlException, HopMissingPluginsException {
   ...
   * @throws HopMissingPluginsException in case missing plugins were found (details are in the
   *         exception in that case) */
  public void loadXml(Node pipelineNode, String filename, IHopMetadataProvider metadataProvider, boolean setInternalVariables, IVariables variables) throws HopXmlException, HopMissingPluginsException {

    HopMissingPluginsException missingPluginsException = new HopMissingPluginsException(BaseMessages.getString(PKG, "PipelineMeta.MissingPluginsFoundWhileLoadingPipeline.Exception"));
    ...
    try {
      try {
        ...
        //
        capturingTransformPerformanceSnapShots = "Y".equalsIgnoreCase(XmlHandler.getTagValue(infoNode, "capture_transform_performance"));
        transformPerformanceCapturingDelay = Const.toLong(XmlHandler.getTagValue(infoNode, "transform_performance_capturing_delay"), 1000);
  ...
  private boolean hasLoop(TransformMeta transformMeta, TransformMeta lookup, HashSet<TransformMeta> checkedEntries) {
    ...
    for (int i = 0; i < nr; i++) {
      TransformMeta prevTransformMeta = prevTransforms.get(i);
      if (prevTransformMeta != null && (prevTransformMeta.equals(lookup) || (!checkedEntries.contains(prevTransformMeta) && hasLoop(prevTransformMeta, lookup == null ? transformMeta : lookup, checkedEntries)))) {
        hasLoop = true;
        break;
      }
    }
  ...
  public void checkTransforms(List<ICheckResult> remarks, boolean onlySelected, IProgressMonitor monitor, IVariables variables, IHopMetadataProvider metadataProvider) {
    try {
      ...
      ExtensionPointHandler.callExtensionPoint(LogChannel.GENERAL, variables, HopExtensionPoint.BeforeCheckTransforms.id, new CheckTransformsExtension(remarks, variables, this, transforms, metadataProvider));
      ...
      for (int i = 0; i < transforms.length && !stopChecking; i++) {
        ...
        if (infoTransform != null) {
          try {
            ...
            info = null;
            CheckResult cr = new CheckResult(ICheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(PKG, "PipelineMeta.CheckResult.TypeResultError.ErrorOccurredGettingTransformMetaFields.Description", "" + transformMeta, Const.CR + kse.getMessage()), transformMeta);
            remarks.add(cr);
          }
        }
        ...
        try {
          ...
          CheckResult cr = new CheckResult(ICheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(PKG, "PipelineMeta.CheckResult.TypeResultError.ErrorOccurredGettingInputFields.Description", "" + transformMeta, Const.CR + kse.getMessage()), transformMeta);
          remarks.add(cr);
        ...
        if (isTransformUsedInPipelineHops(transformMeta) || getTransforms().size() == 1) {
          ...
          ExtensionPointHandler.callExtensionPoint(LogChannel.GENERAL, variables, HopExtensionPoint.BeforeCheckTransform.id, new CheckTransformsExtension(remarks, variables, this, new TransformMeta[] {transformMeta}, metadataProvider));
          transformMeta.check(remarks, this, prev, input, output, info, variables, metadataProvider);
          ExtensionPointHandler.callExtensionPoint(LogChannel.GENERAL, variables, HopExtensionPoint.AfterCheckTransform.id, new CheckTransformsExtension(remarks, variables, this, new TransformMeta[] {transformMeta}, metadataProvider));
          ...
          if (prev != null) {
            ...
            if (prev.size() > 1) {
              ...
              for (int x = 1; x < sortedNames.length; x++) {
                ...
                if (prevName.equalsIgnoreCase(sortedNames[x])) {
                  // Give a warning!!
                  CheckResult cr = new CheckResult(ICheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(PKG, "PipelineMeta.CheckResult.TypeResultWarning.HaveTheSameNameField.Description", prevName), transformMeta);
                  remarks.add(cr);
                } else {
            ...
            CheckResult cr = new CheckResult(ICheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(PKG, "PipelineMeta.CheckResult.TypeResultError.CannotFindPreviousFields.Description") + transformMeta.getName(), transformMeta);
            remarks.add(cr);
          }
        } else {
          CheckResult cr = new CheckResult(ICheckResult.TYPE_RESULT_WARNING, BaseMessages.getString(PKG, "PipelineMeta.CheckResult.TypeResultWarning.TransformIsNotUsed.Description"), transformMeta);
          remarks.add(cr);
        }
      ...
      if (values.size() > 0) {
        for (IValueMeta v : values.keySet()) {
          ...
          CheckResult cr = new CheckResult(ICheckResult.TYPE_RESULT_WARNING, BaseMessages.getString(PKG, "PipelineMeta.CheckResult.TypeResultWarning.Description", v.getName(), message, v.getOrigin()), findTransform(v.getOrigin()));
          remarks.add(cr);
        }
      } else {
      ...
      ExtensionPointHandler.callExtensionPoint(LogChannel.GENERAL, variables, HopExtensionPoint.AfterCheckTransforms.id, new CheckTransformsExtension(remarks, variables, this, transforms, metadataProvider));
    } catch (Exception e) {
    ...
    if (searchTransforms) {
      ...
      for (int i = 0; i < nrTransforms(); i++) {
        ...
        if (transformMeta.getDescription() != null) {
          stringList.add(new StringSearchResult(transformMeta.getDescription(), transformMeta, this, BaseMessages.getString(PKG, "PipelineMeta.SearchMetadata.TransformDescription")));
        }
  ...
  protected void setInternalEntryCurrentDirectory(IVariables variables) {
    variables.setVariable(Const.INTERNAL_VARIABLE_ENTRY_CURRENT_FOLDER, variables.getVariable(StringUtils.isNotEmpty(filename) ? Const.INTERNAL_VARIABLE_PIPELINE_FILENAME_DIRECTORY : Const.INTERNAL_VARIABLE_ENTRY_CURRENT_FOLDER));
  }
