...
public abstract class BaseFileInputTransform<Meta extends BaseFileInputMeta, Data extends BaseFileInputTransformData> extends BaseTransform<Meta, Data>
  ...
  public boolean init() {
    ...
    if ((previousResult == null || resultFiles == null || resultFiles.size() == 0) && data.files.nrOfMissingFiles() > 0 && !meta.inputFiles.acceptingFilenames
  ...
  private void rejectCurrentFile(String errorMsg) {
    if (StringUtils.isNotBlank(meta.errorHandling.fileErrorField) || StringUtils.isNotBlank(meta.errorHandling.fileErrorMessageField)) {
      ...
      int errorFileIndex =
          (StringUtils.isBlank(meta.errorHandling.fileErrorField)) ? -1
              : BaseFileInputTransformUtils.addValueMeta(getTransformName(), rowMeta, this.resolve(meta.errorHandling.fileErrorField));

      int errorMessageIndex =
          StringUtils.isBlank(meta.errorHandling.fileErrorMessageField) ? -1
              : BaseFileInputTransformUtils.addValueMeta(getTransformName(), rowMeta, this.resolve(meta.errorHandling.fileErrorMessageField));
