...
public abstract class BaseHopServerPlugin extends BaseHttpServlet implements IHopServerPlugin, IHopServerRequestHandler {
  ...
  private class HopServerRequestImpl implements IHopServerRequest {
    ...
    @Override
    public IHopServerResponse respond(int status) {
      if (status >= 400) {
        try {
      ...
      return new IHopServerResponse() {
        @Override
        public void with(String contentType, IWriterResponse response) throws IOException {
          resp.setContentType(contentType);
          response.write(resp.getWriter());
        }

        @Override
        public void with(String contentType, IOutputStreamResponse response) throws IOException {
          resp.setContentType(contentType);
