...
public class GetExecutionInfoServlet extends BaseHttpServlet implements IHopServerPlugin {
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    try {
      ...
      if (log.isDebug()) {
        logDebug("Execution information requested of type " + typeString + " and location " + locationName);
      }
      ...
      try {
        switch (type) {
          ...
          case STATE_LOGGING: {
            ...
            String loggingText = location.getExecutionInfoLocation().getExecutionStateLoggingText(id, sizeLimit);
          ...
          case DELETE: {
            String id = request.getParameter(PARAMETER_ID);
            if (StringUtils.isEmpty(id)) {
              throw new HopException("Please specify the ID of the execution to delete with parameter 'id'");
            }
            ...
            break;
          default:
            StringBuilder message = new StringBuilder("Unknown update type: " + type + ". Allowed values are: ");
  ...
  private void outputSuccessAsJson(PrintWriter out, boolean success) throws HopException {
    try {
      ...
      throw new HopException("Error writing success boolean value as JSON to servlet output stream", e);
    }
  }
